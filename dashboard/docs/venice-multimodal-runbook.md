...
**A chat, Anthropic or Responses stream that Venice answered 200 is settled in the request, however it ends.** With Venice's usage frame it is charged the exact cost. Without one (the client closed the stream, the stream broke or hit the routes' 270 s deadline, Venice left the frame out) it is charged the input estimate plus the output it forwarded, counted as it streamed ([`lib/venice/stream-output-meter.ts`](../src/lib/venice/stream-output-meter.ts): the larger of the frames that carried text and the text's UTF-8 size / 4), never more than the hold, and the rest of the hold goes straight back. Those rows carry `metadata.pricingPolicy = managed_venice_observed_output_capture`. A refused request's hold is released; if that release fails, `managed_venice_chat_release_failed` is filed. The Cloudflare Worker does the same through `/internal/settle` (it sends `observedOutputTokens`), retries settles until they land, and chooses each hold's reference itself so it can release the hold after any failure that follows authorize.
...
- A Responses hold whose upstream outcome is unknown (the dispatch threw, or a 5xx) is left to an operator until the hold expires, then **released**. Every other Responses item follows a 200 and is captured.
