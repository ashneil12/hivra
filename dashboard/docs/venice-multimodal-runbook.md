...
**Holds that the request could not settle** are settled by the stale-hold sweep ([`lib/venice/reservation-sweep.ts`](../src/lib/venice/reservation-sweep.ts)), run by the daily `/api/cron/managed-venice-reconciliation`:
- Every hold has an `expires_at`: one hour for media, a day for chat and Responses. Each media hold also records what a success is charged (`metadata.captureOnSuccessMicroUsd`).
- Venice answered 2xx but the request could not settle it (a failed capture, no usage frame, a stream the client cancelled, a body that couldn't be read): the hold is **captured**, at the catalog price for media, or for chat at the input estimate plus at most 4,096 output tokens per choice (never more than the pre-request estimate; the hold itself can cover the model's whole output limit). A 2xx whose body can't be read is captured in the request itself.
- Venice refused the request and the in-request release failed (`managed_venice_media_release_failed`): the hold is **released**.
- An expired hold with no reconciliation item (the function died mid-request) is **captured**: nothing proves Venice didn't run it.
- A client-cancelled stream waits until its hold expires; a Responses hold whose upstream outcome is unknown, and a hold created before holds expired, are left to an operator.
Swept captures write a usage row (`metadata.pricingPolicy = managed_venice_hold_sweep_capture`) and a `usage_capture` event; the chat reconciliation cron skips those rows, since there are no token counts to re-cost.
