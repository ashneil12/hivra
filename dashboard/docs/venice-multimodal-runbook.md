...
Every managed-Venice request goes to Venice with **Hivra's** upstream key, so Venice bills Hivra whether or not the user can pay. The chat path reserves per call (`reserveManagedVeniceChatRequest` → token-level recost → refund-or-flag). Chat options Venice bills on top of tokens (`venice_parameters.enable_web_search`, `enable_web_scraping`, `enable_x_search`, and the `web_search` / `x_search` tools) are held at their most and charged at Venice's rate from its reported per-request `cost`, or at the published rates without it; X search charged without a reported cost, or any clamped charge, files a `managed_venice_x_search_cost_unreported` / `managed_venice_surcharge_above_ceiling` reconciliation item (key stays live). An unreviewed `venice_parameters` option or tool type is refused with a 400. See [`lib/venice/chat-surcharges.ts`](../src/lib/venice/chat-surcharges.ts). Every paid media route (images, video, audio, embeddings, web search/scrape, and the paid paths of the `[...path]` passthrough) goes through the **media spend gate** in [`lib/venice/media-spend-gate.ts`](../src/lib/venice/media-spend-gate.ts):
...
**Token-wallet captures are compare-and-set.** A capture on a `$HermesOS` wallet debits token lots oldest first, and each lot write only lands if the lot still holds the value the capture read (`debitHermesosLots` in [`lib/billing/managed-venice-wallets.ts`](../src/lib/billing/managed-venice-wallets.ts)). A capture that loses a lot to a concurrent one re-reads and debits what it still owes. Before this, ten images captured at once on a $1.00 lot all wrote $0.95, so the wallet paid for one image in ten. Card wallets were never affected: each debit is its own ledger row under a balance trigger.
