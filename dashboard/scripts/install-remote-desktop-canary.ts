...
async function main() {
  ...
  if (!apply) {
    process.stdout.write(`${JSON.stringify({ protocol: "hivra-remote-desktop-canary-install-plan-v1", agentId, controlOrigin: "https://canary.hermesos.cloud", mutatesCanaryGuest: false })}\n`);
    return;
  }
  ...
  const response = await fetch("https://canary.hermesos.cloud/api/ops/hivra/remote-desktop-guest", {
    ...
    body: JSON.stringify({ action: "install", agentId }),
  });
  ...
  if (!result || result.agentId !== agentId || typeof result.ok !== "boolean") {
    throw new Error("Canary did not return a bound desktop install receipt.");
  }
...
void main().catch(() => {
  process.stdout.write(`${JSON.stringify({
    ...
    error: "Remote desktop Canary install command failed.",
  })}\n`);
  process.exitCode = 1;
});
