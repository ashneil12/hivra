...
function handoffHtml(controlOrigin) {
  ...
  const streamProfiles={
    ...
    uhd:{videoBitRate:65000,videoFramerate:60},
    performance:{videoBitRate:12000,videoFramerate:60}
  };
  ...
  const applyStreamingMode=()=>{
    try{
      const child=desktop.contentWindow;
      const app=child&&child.app;
      const profile=streamProfiles[streamingMode];
      if(!app||typeof app!=='object'||!profile)return false;
