-- Transactional stale billing-state reconciliation.
--
-- Used by /api/cron/reconcile-subscription-state after Stripe has proven a
-- paid-looking row is abandoned/terminal. The function is deliberately small
-- and row-locked: it CAS-checks the subscription snapshot the cron observed,
-- resets that row to canonical Free, derives token/yearly entitlement inside
-- the same transaction, and updates instance DB caps without a stale TS-side
-- last-writer race. Container/VM enforcement remains on the existing
-- tier_change_pending + apply-pending-resizes lane.
    ...
    if v_token_tier = 'power' then
      ...
      from (
        ...
        where snapshot.user_id = p_user_id
          and snapshot.token_symbol = config.token_symbol
...
comment on function public.reconcile_stale_subscription_state_to_free(
  ...
  'CAS-reset stale paid subscription rows to Free and atomically align instance DB caps to token/yearly/free entitlement.';
