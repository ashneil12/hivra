-- Admission is separate from bundle publication. It retains the original
-- setup lease, computer identity and exclusive allocation contract. There is
-- no resource purchase, installer dispatch or claim of a running agent here.
create function public.admit_prepared_provider_computer(
...
begin
  ...
  if not found or t.provider_retired_at is not null or t.last_preflight_at is null
    ...
    or not coalesce(t.capabilities @> '{"kind":"provider-vm","provider":"hetzner-cloud","allocation":"exclusive-computer","provisioner":{"configured":true,"ready":true,"version":"2026.08.28.1"}}'::jsonb,false)
...
-- The original native-only launch owns model settings until it terminates.
-- Guard the row, not only the new route, so an old writer or concurrent clear
-- cannot claim a configured model while the guest is still being installed.
create function public.guard_provider_launch_model()
returns trigger language plpgsql security invoker set search_path=public,pg_temp as $$
begin
  if old.computer_substrate='provider-vm' and old.operation_kind='provision'
    and row(new.llm_config,new.llm_api_key_encrypted) is distinct from row(old.llm_config,old.llm_api_key_encrypted) then
    raise exception 'Wait for provider launch to finish before changing model settings' using errcode='55000';
  end if;
  return new;
end;
$$;
create trigger hivra_agents_provider_launch_model_guard before update on public.hivra_agents
  for each row execute function public.guard_provider_launch_model();
revoke all on function public.guard_provider_launch_model() from public,anon,authenticated;

-- A lost reservation/tunnel response must not remain "launching" forever.
-- Only the DB clock and the absence of ANY dispatch grant can close this
-- original operation. It retains the owned computer and all cleanup journals.
create function public.expire_unstarted_provider_agent(p_user_id text,p_agent_id uuid,p_operation_id uuid)
returns boolean language plpgsql security invoker set search_path=public,pg_temp as $$
...
begin
  select * into a from public.hivra_agents where id=p_agent_id and user_id=p_user_id
    ...
    and allocation_operation_id=p_operation_id and operation_kind='provision'
    and status='provisioning' and desired_state='running' for update;
  if not found or a.provider_install_identity is not null
    or a.provider_install_not_after is null or a.provider_install_not_after>clock_timestamp()
    then return false; end if;
  return public.release_hivra_agent_operation(p_user_id,p_agent_id,p_operation_id,
    'The launch window ended before installation started. The original computer is retained; inspect or remove it in Manage. Provider billing may continue.',true);
end;
$$;
revoke all on function public.expire_unstarted_provider_agent(text,uuid,uuid) from public,anon,authenticated;
grant execute on function public.expire_unstarted_provider_agent(text,uuid,uuid) to service_role;
