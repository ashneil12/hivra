-- Master-key rotation must not invalidate launch-request idempotency. New
-- installations emit v2 fingerprints from a dedicated operator-owned HMAC
-- key; existing v1 rows and deployments remain valid until explicitly retired.
alter table public.hivra_launch_model_requests
  drop constraint if exists hivra_launch_model_requests_fingerprint_version_check;
alter table public.hivra_launch_model_requests
  add constraint hivra_launch_model_requests_fingerprint_version_check
...
comment on column public.hivra_launch_model_requests.fingerprint_version is
  'v1 uses the legacy encryption-master coupling; v2 uses the dedicated launch-fingerprint key.';
