-- Private append-only observations. Never readiness, activation or lease release.
alter table public.hivra_agent_attachment_activation_dispatches
  add constraint hivra_activation_operation_identity unique(operation_id,activation_id);
create table public.hivra_agent_attachment_activation_observations (
  observation_id uuid primary key,
  operation_id uuid not null,
  activation_id uuid not null,
  result jsonb not null check(jsonb_typeof(result)='object'),
  created_at timestamptz not null default clock_timestamp(),
  foreign key(operation_id,activation_id) references public.hivra_agent_attachment_activation_dispatches(operation_id,activation_id)
);
create table public.hivra_agent_attachment_activation_observation_outbox (
  observation_id uuid primary key references public.hivra_agent_attachment_activation_observations(observation_id),
  event_kind text not null check(event_kind='service_activation_observed'),
  payload jsonb not null check(jsonb_typeof(payload)='object'),
  created_at timestamptz not null default clock_timestamp()
);
alter table public.hivra_agent_attachment_activation_observations enable row level security;
alter table public.hivra_agent_attachment_activation_observation_outbox enable row level security;
revoke all on public.hivra_agent_attachment_activation_observations,public.hivra_agent_attachment_activation_observation_outbox
  from public,anon,authenticated,service_role;
grant select on public.hivra_agent_attachment_activation_observations,public.hivra_agent_attachment_activation_observation_outbox to service_role;

create function public.record_hivra_attachment_activation_observation(
...
begin
  if p_owner is null or p_operation_id is null or p_expected_generation is null
    ...
    or coalesce(p_result->>'state','') not in ('process_running','service_inactive','activation_unresolved')
    or coalesce(p_result->>'journalPhase','') not in ('preparing','start_requested','service_started','start_failed')
  then return false; end if;
  if p_result->>'state'='process_running' then
