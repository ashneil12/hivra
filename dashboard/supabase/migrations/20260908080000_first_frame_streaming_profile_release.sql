-- Admit the sealed first-frame streaming-profile bundle without invalidating
-- existing computers. This preserves the browser transport and only adds the
-- exact release whose broker consumes the selected mode before document load.
do $migration$
...
begin
  for signature, anchor, addition in select * from (values
    ('public.admit_prepared_provider_computer(text,uuid,bigint,uuid,uuid,text,uuid,uuid,jsonb)',
     '''2026.09.06.4'',''2026.09.07.1'',''2026.09.08.1''',
  ...
  loop
    ...
    if (length(definition) - length(replace(definition, anchor, ''))) / length(anchor) <> 1 then
      raise exception 'First-frame streaming release anchor mismatch: %', signature;
    end if;
