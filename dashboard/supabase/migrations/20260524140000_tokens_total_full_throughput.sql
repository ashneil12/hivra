-- Count tokens_total as FULL throughput (input + output + cache_read +
-- reasoning), matching what the agent WebUI's "Tokens" headline shows.
-- Previously tokens_total was generation-only (input + output), which read
-- ~10x smaller than the WebUI because agents re-read a large cached context
-- each turn (cache_read dominates). tokens_in / tokens_out stay the
-- generation breakdown; only the headline total changes. Identical to the
-- runtime-primary rollup otherwise.
...
begin
  ...
  select
    ...
    -- tokens_total = full throughput: input + output + cache_read + reasoning
