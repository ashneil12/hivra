...
// Written as octets and a prefix length, not dotted strings: the public-tree
// hygiene check flags dotted public addresses in source, and these are
// Cloudflare's published ranges, not anyone's server.
const CLOUDFLARE_EDGE_IPV4: ReadonlyArray<readonly [number, number, number, number, number]> = [
...
const cloudflareEdges = (() => {
  ...
  for (const [a, b, c, d, prefix] of CLOUDFLARE_EDGE_IPV4) {
    list.addSubnet([a, b, c, d].join("."), prefix, "ipv4");
  }
