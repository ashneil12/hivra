...
import { beginProviderNativeCleanup, beginProviderNativeInstall, completeProviderNativeRunning, loadProviderNativeInstallOperation,
  recordProviderNativeCleanup, recordProviderNativeInstallStopped } from "../provider-native-install-store";
...
const h = nativeInstallFixture(), { op, identity, clock, grant } = h;
const access = { mode: "cloudflare-named" as const, hostname: h.context.hostname!, tunnelId: h.context.tunnelId! };
const receipt = { version: 2 as const, identity, state: "failed" as const, stopped: true,
  nativeCleanup: { state: "verified_stopped" as const, bootId: clock.bootId } };
function row() { return { id: op.agentId, user_id: op.userId, operation_id: op.operationId, allocation_operation_id: op.operationId,
  ...
  vmid: null, type: "deepseek-harness", infrastructure_connection_id: h.f.binding.connectionId,
...
describe("private native install and cleanup store", () => {
  ...
  it.each([false, true])("reads cancellation intent without returning cached cleanup authority: %s", async cancelled => {
    ...
    await expect(loadProviderNativeInstallOperation(op)).resolves.toMatchObject({ runtime: "deepseek-harness", identity,
      operation: op, scope: h.scope, cancellationRequested: cancelled });
    expect(mockFrom.mock.calls.map(c => c[0])).toEqual(["hivra_agents", "infrastructure_capacity_orders", "infrastructure_first_boot_enrollments", "hivra_provider_native_cleanup"]);
  ...
  it("keeps the default public operation loader closed to DeepSeek before reading credentials or order", async () => {
  ...
  it("keeps a v1 record out of the native loader", async () => {
    mockRead.mockResolvedValue({ data: { ...row(), type: "codex" }, error: null });
    await expect(loadProviderNativeInstallOperation(op)).rejects.toThrow("could not be verified");
    expect(mockFrom).toHaveBeenCalledTimes(1);
  });
  ...
  it.each([{ outcome: "dispatch", dispatchBudgetMs: 30000 }, { outcome: "observe" }, { outcome: "rejected" }])("preserves native one-use dispatch result %j", async data => {
    mockRpc.mockResolvedValue({ data, error: null });
    await expect(beginProviderNativeInstall(op, identity, access)).resolves.toEqual(data);
    expect(mockRpc).toHaveBeenCalledWith("begin_hivra_provider_native_install", { p_user_id: op.userId, p_agent_id: op.agentId,
  ...
  it.each([null, true, { outcome: "dispatch" }, { outcome: "dispatch", dispatchBudgetMs: 31000 }])("rejects malformed dispatch grant %j", async data => {
    mockRpc.mockResolvedValue({ data, error: null });
    await expect(beginProviderNativeInstall(op, identity, access)).rejects.toThrow("could not be verified");
  });
  it.each([grant, null])("preserves cleanup grant or rejection %j", async data => {
    mockRpc.mockResolvedValue({ data, error: null });
    await expect(beginProviderNativeCleanup(op, identity)).resolves.toEqual(data);
    expect(mockRpc).toHaveBeenCalledWith("begin_hivra_provider_native_cleanup", { p_user_id: op.userId, p_agent_id: op.agentId,
  ...
  it.each([true, {}, { ...grant, budgetMs: 60000 }, { ...grant, observationId: "foreign" }, { ...grant, receipt }])("does not invent a cleanup grant from %j", async data => {
    mockRpc.mockResolvedValue({ data, error: null });
    await expect(beginProviderNativeCleanup(op, identity)).rejects.toThrow("could not be verified");
  });
  it.each(["agent", "operation", "bundle", "closure", "version"])("rejects changed native %s before any RPC", async field => {
    ...
    if (field === "closure") Object.assign(changed.nativeCleanup, { closureSha256: "f".repeat(64) });
    if (field === "version") Object.assign(changed, { version: 1 });
    await expect(beginProviderNativeInstall(op, changed, access)).rejects.toThrow();
    await expect(beginProviderNativeCleanup(op, changed)).rejects.toThrow();
    expect(mockRpc).not.toHaveBeenCalled();
  });
  it("rejects v2 identities/receipts at the unchanged v1 store entrypoints", async () => {
  ...
  it.each([null, { ...access, hostname: "https://foreign.example.test" }, { ...access, tunnelId: null },
    { ...access, mode: "direct-https" }, { ...access, secret: "not-allowed" }])("rejects malformed native access before dispatch RPC %j", async value => {
    await expect(beginProviderNativeInstall(op, identity, value as never)).rejects.toThrow("could not be verified");
    expect(mockRpc).not.toHaveBeenCalled();
  });
  it.each([true, false])("records exact immutable stopped outcome separately from cleanup: %s", async data => {
    mockRpc.mockResolvedValue({ data, error: null });
    const pending = { ...receipt, nativeCleanup: { state: "pending" as const } };
    await expect(recordProviderNativeInstallStopped(op, pending, clock)).resolves.toBe(data);
  ...
  it.each([true, false])("records against the captured grant and preserves SQL decision %s", async data => {
    mockRpc.mockResolvedValue({ data, error: null });
    await expect(recordProviderNativeCleanup(op, grant, receipt, clock)).resolves.toBe(data);
    expect(mockRpc).toHaveBeenCalledWith("record_hivra_provider_native_cleanup", { p_user_id: op.userId, p_agent_id: op.agentId,
  ...
  it.each([true, false])("uses the native terminal CAS without a database bearer: %s", async data => {
    mockRpc.mockResolvedValue({ data, error: null });
    const input = { ...op, chatUrl: "https://fixture.hivra.test", ip: "93.184.216.34",
      provisionedAt: "2026-08-31T21:02:00.000Z" };
    await expect(completeProviderNativeRunning(input)).resolves.toBe(data);
    expect(mockRpc).toHaveBeenCalledWith("complete_hivra_provider_native_running", {
  ...
  it.each(["pending", "nonterminal", "stale-boot", "missing-clock", "foreign", "missing-grant"])("does not record invalid cleanup %s", async fault => {
    ...
    if (fault === "pending") Object.assign(value, { nativeCleanup: { state: "pending" } });
    if (fault === "nonterminal") Object.assign(value, { state: "running", stopped: false });
    if (fault === "stale-boot") value.nativeCleanup.bootId = op.agentId;
    if (fault === "foreign") value.identity.operationId = op.agentId;
    await expect(recordProviderNativeCleanup(op, fault === "missing-grant" ? null as never : grant,
      value, fault === "missing-clock" ? undefined as never : clock)).rejects.toThrow("could not be verified");
    expect(mockRpc).not.toHaveBeenCalled();
  });
  it("cannot record a stopped native observation using a stale boot either", async () => {
    await expect(recordProviderNativeInstallStopped(op, receipt, { ...clock, bootId: op.agentId })).rejects.toThrow();
    expect(mockRpc).not.toHaveBeenCalled();
  });
  it("snapshots receipt and grant before the RPC awaits, and redacts database errors", async () => {
    ...
    mockRpc.mockImplementation(async () => {
      mutableReceipt.nativeCleanup.bootId = op.agentId; mutableGrant.observationId = op.agentId;
      return { data: null, error: { message: "private database detail" } };
    });
    await expect(recordProviderNativeCleanup(op, mutableGrant, mutableReceipt, clock)).rejects.toThrow("could not be verified");
    expect(mockRpc).toHaveBeenCalledWith("record_hivra_provider_native_cleanup", expect.objectContaining({ p_observation_id: grant.observationId,
      p_receipt: receipt }));
  });
});
