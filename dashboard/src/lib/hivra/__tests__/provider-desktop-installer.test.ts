...
import type { ProviderDesktopWorkerReceipt } from "@/lib/infrastructure/provider-desktop-worker";
...
function setup(accessMode: "cloudflare-named" | "direct-https" = "cloudflare-named") {
  const h = desktopInstallFixture(accessMode), { context, identity, scope, f, clock } = h;
  let elapsed = 0, state: ProviderDesktopWorkerReceipt["state"] = "running";
  let cleanup: ProviderDesktopWorkerReceipt["desktopCleanup"] = { state: "pending" };
  const deps = {
    ...
    stopped: jest.fn(async (_op: typeof h.op, receipt: ProviderDesktopWorkerReceipt) => {
    ...
    control: jest.fn(async (_input?: unknown) => { void _input; return { hostVerified: true as const, administratorAuthenticated: true as const,
      ...
      receipt: { version: 3 as const, identity, state, stopped: ["cancelled", "failed", "succeeded"].includes(state), desktopCleanup: cleanup } }; }),
    monotonicNow: () => elapsed, controlOrigin: jest.fn(() => "https://canary.hermesos.cloud"),
  };
  return { ...h, deps, advance: (ms: number) => { elapsed = ms; },
    result: (outcome: typeof state, desktop: typeof cleanup = { state: "pending" }) => { state = outcome; cleanup = desktop; } };
}
const call = (h: ReturnType<typeof setup>, action: "status" | "cancel") => advanceProviderDesktopInstaller({ ...h.op, action }, h.deps);

describe("private desktop installer operation adapter", () => {
  it.each(["cloudflare-named", "direct-https"] as const)("dispatches one exact v3 install through original %s authority", async mode => {
    const h = setup(mode);
    await expect(advanceProviderDesktopInstaller(h.request, h.deps)).resolves.toEqual({ stage: "worker_observed", state: "running", stopped: false,
    ...
    expect(h.deps.control).toHaveBeenCalledWith(expect.objectContaining({ action: "start", launch: h.request.launch,
      authority: {computerId: h.op.agentId, controlOrigin: "https://canary.hermesos.cloud",
        access: {mode,hostname: h.context.hostname,tunnelId: h.context.tunnelId}}, scope: h.scope, hostPublicKey: h.f.host.publicKey,
  ...
  it("retains immutable failure without confusing pending desktop cleanup with release", async () => {
    const h = setup(); h.context.identity = h.identity; h.result("failed");
    await expect(call(h, "cancel")).resolves.toMatchObject({ state: "failed", stopped: true, desktopCleanup: "pending", cleanupRecorded: false });
  ...
  it("status freshly observes but never acquires or records cleanup authority, including after cancellation", async () => {
    ...
    h.context.stoppedAt = "2026-08-31T20:00:00Z"; h.context.outcome = "succeeded";
    h.result("succeeded", { state: "verified_stopped", bootId: h.clock.bootId });
    await expect(call(h, "status")).resolves.toMatchObject({ stopped: true, desktopCleanup: "verified_stopped", cleanupRecorded: false, cancellationRequested: true });
  ...
  it.each(["start", "status", "cancel"] as const)("recovers %s without current assets or relaying launch/model credentials", async action => {
    ...
    h.deps.controlOrigin.mockImplementation(() => { throw new Error("Current config unavailable"); });
    await advanceProviderDesktopInstaller({ ...h.request, action }, h.deps);
    expect(h.deps.bundle).not.toHaveBeenCalled(); expect(h.deps.begin).not.toHaveBeenCalled();
  ...
  it.each(["desiredState", "cancellationRequested"])("a repeated start resumes cancellation when %s is set", async field => {
    ...
    await advanceProviderDesktopInstaller(h.request, h.deps);
  ...
  it.each(["status", "cancel", "deleted-start"])("keeps an unstarted reservation non-authoritative for %s", async mode => {
    ...
    await expect(mode === "deleted-start" ? advanceProviderDesktopInstaller(h.request, h.deps) : call(h, mode as "status" | "cancel"))
  ...
  it.each(["tunnel", "origin", "runtime", "profile", "control-origin", "computer", "modelKey", "no-access"])("rejects %s before provider access or consuming a dispatch grant", async field => {
    const h = setup();
    ...
    if (field === "profile") Object.assign(h.context, { computerProfile: "omarchy" });
    if (field === "control-origin") h.deps.controlOrigin.mockReturnValue("https://other.example.test");
    if (field === "computer") h.request.launch.computerId = h.op.operationId;
    ...
    await expect(advanceProviderDesktopInstaller(h.request, h.deps)).rejects.toThrow();
  ...
  it.each(["load", "begin", "boot", "verify", "bootstrap", "cleanupBegin"] as const)("does not refresh the original deadline after %s", async step => {
    ...
    await expect(step === "cleanupBegin" ? call(h, "cancel") : advanceProviderDesktopInstaller(h.request, h.deps))
  ...
  it("does not acquire grants or keys while the original provider is still converging", async () => {
    ...
    await expect(advanceProviderDesktopInstaller(h.request, h.deps)).resolves.toEqual({ stage: "waiting_for_provider" });
