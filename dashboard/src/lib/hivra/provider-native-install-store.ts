...
import { parseProviderNativeWorkerIdentity, parseProviderNativeWorkerReceipt, parseProviderNativeAccess,
  type ProviderNativeAccess, type ProviderNativeWorkerIdentity, type ProviderNativeWorkerReceipt } from "@/lib/infrastructure/provider-native-worker";
import type { ProviderGuestClock } from "@/lib/infrastructure/provider-guest-bundle";
import { loadProviderNativeInstallBinding, ProviderAgentInstallStoreError,
...
export type ProviderNativeCleanupGrant = z.infer<typeof Grant>;
function database() {
  if (!supabaseAdmin) throw new ProviderAgentInstallStoreError();
  return supabaseAdmin;
}
function checkedIdentity(input: ProviderAgentInstallOperation, identity: ProviderNativeWorkerIdentity) {
  const current = Operation.parse(input), checked = parseProviderNativeWorkerIdentity(identity);
  if (checked.agentId !== current.agentId || checked.operationId !== current.operationId) throw new Error();
  return { current, checked };
}
function checkedReceipt(input: ProviderAgentInstallOperation, receipt: ProviderNativeWorkerReceipt, clock: ProviderGuestClock) {
  const { current, checked: identity } = checkedIdentity(input, receipt.identity);
  const checked = parseProviderNativeWorkerReceipt(`HIVRA_PROVIDER_WORKER_V1 ${JSON.stringify(receipt)}\n`, identity, clock);
  if (!checked.stopped) throw new Error();
  return { current, checked };
}

/** Private v2 recovery only. The durable cancellation flag is intent, not a
 * reusable cleanup receipt. Expired/lost grants must still suppress readiness. */
export async function loadProviderNativeInstallOperation(input: ProviderAgentInstallOperation) {
  try {
    const current = Operation.parse(input), context = await loadProviderNativeInstallBinding(current);
    const { data, error } = await database().from("hivra_provider_native_cleanup")
    ...
    if (data !== null) {
      ...
      const identity = checkedIdentity(current, journal.identity as ProviderNativeWorkerIdentity).checked;
      if (journal.agent_id !== current.agentId || journal.user_id !== current.userId || journal.operation_id !== current.operationId
        || JSON.stringify(identity) !== JSON.stringify(parseProviderNativeWorkerIdentity(context.identity))) throw new Error();
    }
    return { ...context, cancellationRequested: data !== null };
  } catch { throw new ProviderAgentInstallStoreError(); }
}
export type ProviderNativeInstallContext = Awaited<ReturnType<typeof loadProviderNativeInstallOperation>>;

export async function beginProviderNativeInstall(input: ProviderAgentInstallOperation, identity: ProviderNativeWorkerIdentity, access: ProviderNativeAccess) {
  try {
    const { current, checked } = checkedIdentity(input, identity), bound = parseProviderNativeAccess(access);
    const { data, error } = await database().rpc("begin_hivra_provider_native_install", {
...
export async function recordProviderNativeInstallStopped(input: ProviderAgentInstallOperation,
  receipt: ProviderNativeWorkerReceipt, clock: ProviderGuestClock) {
  try {
...
export async function beginProviderNativeCleanup(input: ProviderAgentInstallOperation, identity: ProviderNativeWorkerIdentity) {
  try {
    const { current, checked } = checkedIdentity(input, identity);
    const { data, error } = await database().rpc("begin_hivra_provider_native_cleanup", {
...
export async function recordProviderNativeCleanup(input: ProviderAgentInstallOperation, grant: ProviderNativeCleanupGrant,
  receipt: ProviderNativeWorkerReceipt, clock: ProviderGuestClock) {
  try {
    const token = Grant.parse(grant), { current, checked } = checkedReceipt(input, receipt, clock);
    if (checked.nativeCleanup.state === "pending") throw new Error();
    const { data, error } = await database().rpc("record_hivra_provider_native_cleanup", {
...
/** Native readiness has no database bearer. Keep the shared v1
 * complete_hivra_agent_running NULL=preserve semantics unchanged and use the
 * native terminal CAS that explicitly clears any staged legacy token. */
export async function completeProviderNativeRunning(input: ProviderAgentInstallOperation & {
  ...
  try {
    ...
    const { data, error } = await database().rpc("complete_hivra_provider_native_running", {
      p_user_id: current.userId, p_agent_id: current.agentId, p_operation_id: current.operationId,
