...
describe("activity feed normalization",()=>{
  ...
  it("titles an in-place runtime update as an update, not a restart",()=>{
    ...
    expect(snapshot.events[0]).toMatchObject({kind:"lifecycle",title:"Computer runtime updated",outcome:"unknown",needsAttention:false});
  });
