...
export const article: BlogArticle = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "You may see a tool-call confirmation in the terminal while the actual response text appears only in Telegram. That is expected for channel replies. If you want to drive the exact terminal interface from a browser instead, Hivra's [Claude Code agent](/agents/claude-code) provides browser access to chat, terminal, and files. On Hivra, a Claude Code agent also has its own Telegram connect, in its Telegram tab under Manage, which runs on the agent's computer without the plugin setup. Anthropic's channel plugin described here is the route when you host Claude Code yourself.",
      ],
    },
    {
  ...
  faqs: [
    ...
    },
    {
      ...
      a: "On Hivra, a Claude Code agent has its own Telegram connect in its Telegram tab, under Manage: paste a bot token from BotFather, open the pairing link, and messages to the bot run on the agent's computer. It is not automatic, since you bring your own bot. The setup in this guide, Anthropic's official channel plugin, is the route when you host Claude Code yourself.",
    },
  ],
  relatedArticles: [
    {
