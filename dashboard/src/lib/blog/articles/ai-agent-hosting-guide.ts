...
export const article: BlogArticle = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        `Cash costs first, then the honest part: your time.\n\n| Option | Cash per month | Setup time | Maintenance | Keeps running with your laptop closed |\n|---|---|---|---|---|\n| Own hardware | ~$3-8 electricity | 1-3 hours | Yours, plus home internet risk | Yes, if the box stays awake |\n| VPS (DIY) | $5-10 | 1-2 hours | Yours: patches, security, restarts | Yes |\n| Serverless | Varies, often near $0 idle | Hours to days of glue code | Yours, and the architecture fights you | N/A, no persistent process |\n| Managed (Hivra) | From ${ENTRY_PLAN_PRICE} | Pick an agent and sign in | The server is run for you | The computer, yes; Claude Code and Codex runs need Telegram or tmux |\n\nOn Hivra specifically: the ${ENTRY_PLAN_PRICE}/month plan is ${ENTRY_PLAN_SIZE}, and the ${LARGER_PLAN_PRICE}/month plan is ${LARGER_PLAN_SIZE}. Paid plans are not paused for inactivity, and both come with a ${MONEY_BACK_GUARANTEE}. ${SERVER_SIDE_AGENTS_KEEP_WORKING} For Claude Code and Codex, start long runs from the agent's Telegram tab or inside tmux, because a run started in the browser chat stops when you close that tab. Full details on [the pricing page](/pricing).`,
        "Two things the table cannot show. First, DIY cash costs are floors, not totals: an hour of your time spent patching a box has a price even if no invoice arrives. Second, model usage dwarfs hosting for heavy users either way. A busy agent can spend more on tokens in a day than the VPS costs in a month. If you want numbers for your own setup, [the hosting cost calculator](/tools/ai-agent-hosting-cost-calculator) lets you compare DIY and managed with your actual usage.",
      ],
    },
    {
  ...
  faqs: [
    ...
    },
    {
      ...
      a: "Many agent CLIs run in about 1 GB, though Anthropic lists 4 GB for Claude Code. The moment the agent drives a real browser, budget 2-4 GB for the browser alone, because Chrome is the heaviest process on the box. On the DIY route that means the cheapest VPS tier is often too small for browser work; size up or the browser gets killed mid-task.",
    },
    {
    ...
    },
    {
      ...
      a: "It can be, if you treat it like the sensitive box it is: SSH keys only, no public ports without auth in front, the agent isolated on its own machine, and patches applied on a schedule. The agent holds your logged-in sessions and executes commands, so an exposed dashboard is the main thing to never allow.",
    },
  ],
  relatedArticles: [
    {
