...
describe("applyLiveUpdate", () => {
  ...
  it("launches Proxmox-backed live updates through the Proxmox host into the guest VM", async () => {
    ...
    expect(updateEq).toHaveBeenCalledWith("id", "66244f0e-e385-4c3d-88c2-c07e005cb2d2");
    ...
    const wrapperScript = extractEmbeddedScript(
      innerScript,
      "/tmp/hermes-update-wrapper-66244f0e-e385-4c3d-88c2-c07e005cb2d2.sh"
    );
    expect(wrapperScript).toContain("/opt/hermes/instances");
    expect(wrapperScript).toContain("! -name \"66244f0e-e385-4c3d-88c2-c07e005cb2d2\"");
  ...
  it("resolves the bastion host env from the instance's host (not HERMES_PROXMOX_TARGET)", async () => {
    ...
    await applyLiveUpdate(
      {
        id: "5b60dfb1-f09b-4b98-8eab-10a01afcd2f3",
        user_id: "user-123",
        provider: "openai",
        hetzner_server_id: null,
        host_id: null,
  ...
  it("surfaces a structured error when the host slug has no matching PROXMOX_<SLUG>_* env", async () => {
    ...
    const result = await applyLiveUpdate(
      {
        id: "19c931c4-4357-4d5e-827a-f64bfc2af973",
        user_id: "user-123",
        provider: "openai",
        hetzner_server_id: null,
        host_id: null,
  ...
  it("uses the WebUI bootstrap (not the gateway agent script) when instance.backend === 'webui'", async () => {
    ...
    const result = await applyLiveUpdate(
      {
        id: "9b1e5f90-fc42-4c55-8729-b24abdfbd952",
        user_id: "user-123",
        provider: "openai",
        backend: "webui",
        api_key_encrypted: "enc-api-key",
    ...
    expect(buildWebUIProvisioningArtifacts).toHaveBeenCalledWith(
      expect.objectContaining({
        instanceId: "9b1e5f90-fc42-4c55-8729-b24abdfbd952",
        containerName: "agent-9b1e5f90-fc42-4c55-8729-b24abdfbd952",
        fqdn: "localhost",
        cpuLimit: 2,
        ramLimit: 4096,
  ...
  it("nulls personaSoulPrompt and pins the persisted operatoros image for an Operator OS WebUI update", async () => {
    ...
    const result = await applyLiveUpdate(
      {
        id: "eea9546a-ab95-46f5-8579-b5681305c1b7",
        user_id: "user-123",
        provider: "openai",
        backend: "webui",
        api_key_encrypted: "enc-api-key",
  ...
  it("fails closed when a legacy Operator OS row has no pinned image", async () => {
    ...
    await expect(applyLiveUpdate(
      {
        id: "eea9546a-ab95-46f5-8579-b5681305c1b7-noimage",
        user_id: "user-123",
        provider: "openai",
        backend: "webui",
        api_key_encrypted: "enc-api-key",
  ...
  it("drops persisted browser-sidecar opt-in during WebUI live update unless the deployment gate is enabled", async () => {
    ...
    await applyLiveUpdate(
      {
        id: "9b1e5f90-fc42-4c55-8729-b24abdfbd952-sidecar-gated",
        user_id: "user-123",
        provider: "openai",
        backend: "webui",
        hetzner_server_id: 1,
  ...
  it("keeps persisted browser-sidecar opt-in during WebUI live update when the deployment gate is enabled", async () => {
    ...
    await applyLiveUpdate(
      {
        id: "9b1e5f90-fc42-4c55-8729-b24abdfbd952-sidecar-enabled",
        user_id: "user-123",
        provider: "openai",
        backend: "webui",
        hetzner_server_id: 1,
  ...
  it("drops persisted browser-sidecar opt-in during WebUI live update when the tier no longer qualifies", async () => {
    ...
    await applyLiveUpdate(
      {
        id: "9b1e5f90-fc42-4c55-8729-b24abdfbd952-sidecar-tier-dropped",
        user_id: "user-123",
        provider: "openai",
        backend: "webui",
        hetzner_server_id: 1,
  ...
  it("enables browser-sidecar by DEFAULT for a Pro+ WebUI instance with no explicit opt-in", async () => {
    ...
    await applyLiveUpdate(
      {
        id: "9b1e5f90-fc42-4c55-8729-b24abdfbd952-sidecar-default",
        user_id: "user-123",
        provider: "openai",
        backend: "webui",
        hetzner_server_id: 1,
  ...
  it("honors an explicit opt-out (browserSidecarEnabled: false) on a Pro+ WebUI instance", async () => {
    ...
    await applyLiveUpdate(
      {
        id: "9b1e5f90-fc42-4c55-8729-b24abdfbd952-sidecar-optout",
        user_id: "user-123",
        provider: "openai",
        backend: "webui",
        hetzner_server_id: 1,
...
describe("applyLiveUpdate in-flight turn gate", () => {
  ...
  it("gives a gated Proxmox launch its 90 s plus the gate's worst case", async () => {
    ...
    await applyLiveUpdate(
      {
        ...row("5213dffa-deb5-45fe-8922-1550676d268d"),
        hetzner_server_id: null,
        host_id: null,
        config: {
          infrastructure: {
