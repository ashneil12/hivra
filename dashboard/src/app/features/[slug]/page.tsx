...
const FEATURES: Record<string, FeatureData> = {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Hivra hosts the memory layer on dedicated cloud infrastructure. Daily encrypted backups run automatically. If you switch to a new computer, your agent's full memory is exactly where you left it. Nothing is tied to a local process.",
        ],
      },
      {
    ...
    faqs: [
      {
        ...
        a: "Your agent's memory persists for the life of your subscription. There is no expiry. Daily backups ensure you are always covered against data loss.",
      },
      {
        q: "Can I reset or wipe an agent's memory?",
        a: "Yes. You can reset the full memory store or delete individual memories from the dashboard. Selective removal is supported — you do not have to wipe everything to correct one wrong memory.",
      },
      {
        q: "Does memory work across different agent profiles?",
        a: "Each profile has isolated memory by default. Cross-agent references are supported on the Power plan via an explicit coordination mechanism — not automatic sharing.",
      },
      {
        q: "What format is the memory export?",
        a: "One click from the agent's page downloads your data — chats plus memory — as a single structured JSON file. The schema is versioned and self-describing, so your data is always yours to keep, read, or move.",
      },
      {
        q: "Is the memory encrypted?",
        a: "Yes. Memory is encrypted at rest and in transit. Backups use the same encryption.",
      },
      {
        q: "Can the agent's memory get too large and slow things down?",
        a: "Typical agent memory after months of active use is in the hundreds of megabytes — small enough that retrieval latency is not a concern. The memory retrieval system uses vector similarity search, which scales well past hundreds of thousands of stored items.",
      },
    ],
  },
    ...
    faqs: [
      {
        ...
        a: "Hermes agents use a pre-configured headless Chromium browser. It runs inside the isolated container on your Hivra server.",
      },
      {
      ...
      },
      {
        ...
        a: "For most standard websites, yes. For sites with aggressive anti-bot protection (Cloudflare enterprise tiers, PerimeterX), results vary. Residential proxy credentials can be configured for these cases.",
      },
      {
        q: "Can I see screenshots from what the agent did in the browser?",
        a: "Yes. The agent logs browser actions including screenshots to the task history in the dashboard. You can review exactly what the agent saw and did on each run.",
      },
      {
  ...
  "multi-agent": {
    ...
    tagline: "Researcher. Operator. Support. All on one instance.",
    intro: [
    ...
    sections: [
      ...
      {
        heading: "Isolated memory, shared infrastructure",
        paragraphs: [
    ...
    faqs: [
      {
        ...
        a: "Yes. Free includes 1 active agent, Pro includes 3, and Power includes 5. Those slot limits match the compute pool so the dashboard and provisioning gate stay consistent.",
      },
      {
      ...
      {
        q: "What is the difference between Pro and Power for multi-agent use?",
        a: "Pro gives you 3 active agents on 2 vCPU and 4 GB RAM. Power gives you 5 active agents on 4 vCPU and 8 GB RAM, plus more headroom for overlapping scheduled work.",
      },
      {
  ...
  "no-docker-hosting": {
    ...
    tagline: "No VPS. No Docker. No config. 5 minutes.",
    intro: [
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Networking and SSL are handled. Your agent's web interface is accessible over HTTPS at your Hivra subdomain immediately. Bringing a custom domain takes two DNS records and about 15 minutes — no Nginx config files.",
    ...
    relatedFeatures: [
      { slug: "persistent-memory", title: "Persistent Memory" },
      { slug: "openclaw-alternative", title: "OpenClaw Migration" },
      { slug: "multi-agent", title: "Multi-Agent" },
    ],
    relatedBlog: [
      { slug: "self-hosting-hermes-guide", title: "How to self-host Hermes Agent" },
    ...
    faqs: [
      ...
      },
      {
        ...
        a: "No. Hivra is purpose-built managed infrastructure for Hermes agents: pre-configured container runtime, pre-installed browser automation, automatic memory backup, managed updates, and a dashboard purpose-built for Hermes. A generic VPS does none of this for you.",
      },
      {
        q: "What region is my server in?",
        a: "At signup you choose from US East, US West, EU Central, and EU West. Agent latency to AI provider APIs is optimized based on region choice.",
      },
      {
        q: "What happens if Hermes releases an update that breaks my configuration?",
        a: "We test all Hermes updates before rolling them out and document breaking changes. If an update conflicts with your configuration, you are notified before it applies and given a window to review.",
      },
    ],
  },
  ...
  "openclaw-alternative": {
    ...
    tagline: "Your OpenClaw setup moves over intact. In minutes.",
    intro: [
      ...
      "Hivra is the managed alternative. Same agent power — browser automation, tool use, memory, scheduling — hosted on our infrastructure. We handle updates, monitoring, restarts, and backups.",
    ],
    sections: [
      {
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Hivra is built for exactly these situations. You bring your API key — the same one you were using with OpenClaw — and get a managed cloud environment that runs the agent for you.",
        ],
      },
      {
      ...
      },
      {
        ...
        paragraphs: [
          "Browser automation: both support headless browser control. Hivra offers four backends (Browserbase, Browser Use cloud, Chrome CDP, local Chromium) plus SSRF safeguards. OpenClaw runs from your local machine IP.",
          "Memory: OpenClaw has session-level context but no built-in long-term memory store. Hivra implements USER.md and MEMORY.md for persistent context, plus Skill Documents in the agentskills.io format for procedural memory across sessions.",
          "Scheduling: OpenClaw does not support autonomous scheduled tasks. Hivra has full cron scheduling with event hooks for conditional triggering.",
          "Multi-agent: OpenClaw runs one agent at a time. Hivra supports 3 active agents on Pro and 5 on Power, with each agent keeping its own memory and tools.",
          "Model support: OpenClaw focuses on Anthropic's Claude family (the Node.js runtime is optimized for it). Hivra supports Claude Haiku 4.5, Sonnet 4.6, Opus 4.6, GPT-5.4, GPT-5 mini, and 300+ OpenRouter models.",
        ],
      },
    ],
    bullets: [
    ...
    faqs: [
      {
        ...
        a: "OpenClaw is open-source desktop software you self-host. Hivra is managed cloud hosting for Hermes agents. The agent capabilities are similar — browser automation, Claude access, tool use — but Hivra adds persistent memory, scheduling, multi-agent coordination, and removes all the infrastructure work.",
      },
      {
        q: "Can I migrate my existing OpenClaw setup to Hivra?",
        a: "Yes. Agent prompts, instructions, and skill configurations transfer directly. Custom tool directories can be imported via the dashboard import flow.",
      },
      {
        q: "Do I lose anything by switching from OpenClaw to Hivra?",
        a: "You gain managed hosting, persistent memory, automatic backups, scheduling, and zero maintenance. The main trade is that you are no longer running locally — which means your API key is stored on a server rather than your machine. Local browser sessions and stored credentials do not migrate.",
      },
      {
        q: "I built custom tools for OpenClaw. Will they work on Hivra?",
        a: "Custom tools written as Tool Documents in the agentskills.io format import directly. Tools that depend on local machine resources (local files, running processes on your machine) will need to be adapted to work in a cloud container environment.",
      },
    ],
  },
  ...
  "scheduled-tasks": {
    ...
    sections: [
      ...
      {
        heading: "Examples running in the wild",
        paragraphs: [
    ...
    faqs: [
      {
        ...
        a: "Go to Scheduled Tasks in the dashboard, write the task instruction, set the schedule in natural language or cron format, and activate. The agent handles execution automatically at the specified time.",
      },
      {
      ...
      },
      {
        ...
        a: "Failed tasks are logged with error details. You can configure failure alerts per task. The task retries on its next scheduled run by default, or you can configure immediate retry behavior.",
      },
      {
      ...
      },
      {
        ...
        a: "No limit on the number of tasks. Concurrent execution depends on your compute plan — tasks that run simultaneously share the instance's vCPU and RAM. On the Pro plan, 2-3 concurrent resource-intensive tasks is the practical limit.",
      },
    ],
  },
};
