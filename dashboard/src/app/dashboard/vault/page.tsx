...
// Autofocus opens the soft keyboard over the form on phones, so only desktop gets it.
function allowFieldAutoFocus() {
...
export default function VaultPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} style={{ maxWidth: 1100, margin: "1rem auto 5rem", padding: "clamp(1rem, 5vw, 3rem)", paddingTop: "calc(var(--dashboard-page-safe-top, env(safe-area-inset-top, 0px)) + clamp(1rem, 5vw, 3rem))" }}>
        ...
        >
          ...
          <motion.section variants={sectionVariants}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
              <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "1rem", flex: 1, minWidth: "min(250px, 100%)" }}>
                ...
                {keys.length > 0 && (
                  <input 
                    ...
                    spellCheck={false}
                    enterKeyHint="search"
                    style={{ padding: '8px 12px', fontSize: 12, fontFamily: 'var(--font-mono)', border: '1px solid var(--etched-border)', background: 'var(--bg-surface)', flex: '1 1 160px', maxWidth: 200, minWidth: 0 }}
                  />
                )}
              </div>
              <button 
                onClick={() => setIsAddingMode(!isAddingMode)}
                className="max-md:min-h-[44px] pointer-coarse:min-h-[44px]"
                style={{
                  ...
                  <>
                    <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                      <div style={{ flex: '1 1 200px' }}>
                         ...
                         <input required autoFocus={allowFieldAutoFocus()} autoComplete="off" autoCapitalize="off" autoCorrect="off" spellCheck={false} value={newName} onChange={e => setNewName(e.target.value)} placeholder={"e.g. My Anthropic Key"} style={{ width: '100%', padding: '10px', fontSize: 13, fontFamily: 'var(--font-mono)', border: '1px solid var(--etched-border)' }} />
                      </div>
                      <div style={{ flex: '1 1 200px' }}>
                      ...
                      <div style={{ flex: '2 1 300px' }}>
                         ...
                         <input type="password" required autoComplete="off" autoCapitalize="off" autoCorrect="off" spellCheck={false} value={newKey} onChange={e => setNewKey(e.target.value)} placeholder="sk-..." style={{ width: '100%', padding: '10px', fontSize: 13, fontFamily: 'var(--font-mono)', border: '1px solid var(--etched-border)' }} />
                      </div>
                    </div>
                    <div style={{ marginTop: '1.5rem', display: 'flex', justifyContent: 'flex-end' }}>
                      <button type="submit" disabled={savingKey} className="max-md:min-h-[44px] pointer-coarse:min-h-[44px]" style={{ background: 'var(--ink-black)', color: "var(--bg-surface)", border: 'none', padding: '10px 24px', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                  ...
                  <div style={{ padding: '0.5rem 0' }}>
                    ...
                    {agents.length === 0 ? (
                      <div style={{ padding: '16px 20px', background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.2)', display: 'flex', flexDirection: 'column', gap: 12 }}>
                        ...
                        <div>
                           <Link href="/dashboard/agents" className="max-md:min-h-[44px] pointer-coarse:min-h-[44px]" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--ink-black)', background: 'var(--bg-surface)', border: '1px solid var(--ink-black)', padding: '6px 12px', textDecoration: 'none' }}>
                             Open Agents &rarr;
                           </Link>
                        </div>
                      </div>
                        ...
                        <div style={{ border: '1px solid var(--etched-border)', padding: '1rem', background: newProvider === 'codex' ? 'var(--bg-surface)' : 'transparent', opacity: codexStep !== 'idle' && newProvider !== 'codex' ? 0.4 : 1 }}>
                           ...
                           {newProvider === 'codex' && (
                             <div style={{ paddingTop: '1rem', marginTop: '1rem', borderTop: '1px dashed var(--etched-border)' }}>
                               {codexStep === 'idle' ? (
                                  <button type="button" onClick={handleCodexStart} className="max-md:min-h-[44px] pointer-coarse:min-h-[44px]" style={{ width: '100%', padding: '10px', background: 'var(--vellum-bg)', border: '1px solid var(--ink-black)', color: 'var(--ink-black)', fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                                  ...
                                  <div style={{ background: 'var(--vellum-bg)', padding: '16px', border: '1px solid var(--ink-black)' }}>
                                    <div style={{ marginBottom: 12 }}>
                                      ...
                                      <a href={codexUrl} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', minHeight: 44, marginBottom: 8, padding: '10px 14px', background: 'var(--ink-black)', color: 'var(--bg-surface)', border: '1px solid var(--ink-black)', fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', textDecoration: 'none' }}>
                                        Open sign-in page
                                      </a>
                                      <a href={codexUrl} target="_blank" rel="noreferrer" style={{ display: 'block', fontSize: 12, color: 'var(--ink-black)', fontFamily: 'var(--font-mono)', overflowWrap: 'anywhere', wordBreak: 'break-all' }}>{codexUrl}</a>
                                    </div>
                                    {codexCode && (
                                      <div>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                                          <span style={{ fontSize: 18, fontWeight: 700, fontFamily: 'var(--font-mono)', overflowWrap: 'anywhere' }}>{codexCode}</span>
                                          <button type="button" onClick={handleCodexCopy} style={{ minHeight: 44, padding: '10px 14px', background: 'transparent', border: '1px solid var(--etched-border)', fontSize: 11, cursor: 'pointer', color: codexCopied ? 'var(--green)' : 'var(--text-secondary)' }}>
                                  ...
                                  <div style={{ padding: '12px', color: 'var(--red)', fontSize: 12, border: '1px solid rgba(239,68,68,0.2)' }}>
                                    ...
                                    {codexError}
                                    <button type="button" onClick={() => setCodexStep('idle')} style={{ display: 'block', marginTop: 8, minHeight: 44, padding: '10px 14px', background: 'transparent', border: '1px solid var(--red)', cursor: 'pointer', fontSize: 11 }}>Retry</button>
                                  </div>
                               )}
                             </div>
                           )}
                            ...
                            {filteredKeys.map(k => (
                              <tr key={k.id} style={{ borderBottom: '1px solid var(--etched-border)' }}>
                                ...
                                <td style={{ padding: '14px 20px', textAlign: 'right' }}>
                                  <button onClick={() => handleDeleteKey(k.id, k.name)} disabled={deletingKey === k.id} aria-label={`Delete ${k.name}`} style={{ background: 'transparent', color: '#dc2626', border: 'none', cursor: deletingKey === k.id ? 'wait' : 'pointer' }}>
                        ...
                        {filteredKeys.map(k => (
                          <div key={k.id} style={{ border: '1px solid var(--etched-border)', background: 'var(--bg-surface)', padding: '1.25rem' }}>
                            <div className="flex justify-between items-start gap-3 mb-4">
                              <span className="font-mono font-bold" style={{ fontSize: 14, minWidth: 0, overflowWrap: 'anywhere', paddingTop: 12 }}>{k.name}</span>
                              <button onClick={() => handleDeleteKey(k.id, k.name)} disabled={deletingKey === k.id} aria-label={`Delete ${k.name}`} style={{ background: 'transparent', color: '#dc2626', border: 'none', cursor: deletingKey === k.id ? 'wait' : 'pointer', width: 44, height: 44, flexShrink: 0, padding: 0, margin: '0 -10px 0 0', display: 'grid', placeItems: 'center' }}>
                                {deletingKey === k.id ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
                              </button>
                            </div>
            ...
            <div className="w-full">
              {agents.length === 0 ? (
                ...
                <>
                  ...
                  <div className="hidden lg:block" style={{ border: '1px solid var(--etched-border)', background: "var(--bg-surface)", width: "100%" }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
                      <thead>
                        <tr style={{ background: 'var(--bg-elevated)', borderBottom: '1px solid var(--etched-border)', fontFamily: 'var(--font-mono), monospace', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', opacity: 0.6 }}>
                  ...
                  <div className="lg:hidden flex flex-col gap-6 w-full">
                    {agents.map(agent => {
                      const noSelection = !agentAssignments[agent.id]?.vaultKeyId && !agentAssignments[agent.id]?.honchoVaultKeyId && !agentAssignments[agent.id]?.model;
                      // Selects set back to "Leave current" leave nothing to apply.
                      const hasChanges = changedAgents.has(agent.id) && !noSelection;
                      return (
                      <div key={agent.id} style={{ border: `1px solid ${hasChanges ? 'var(--gold-leaf)' : 'var(--etched-border)'}`, background: 'var(--bg-surface)', padding: '1.25rem' }}>
                        ...
                        <div className="mt-5">
                          <button 
                            type="button"
                            disabled={syncingAgent === agent.id || noSelection}
                          ...
                          {hasChanges && (
                            <p className="mono" style={{ margin: '8px 0 0', fontSize: 11, lineHeight: 1.5, color: 'var(--text-secondary)' }}>
                              Unsaved — tap Apply to update this agent
                            </p>
                          )}
                        </div>
                      </div>
