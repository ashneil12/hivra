...
import { readManagedSessionHistory } from "@/lib/hivra/do-managed-sessions";
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    return noStore(apiSuccess(await readManagedSessionHistory(userId, id.toLowerCase())));
  } catch (error) {
    return managedSessionFailure(error, "/api/hivra/managed-sessions/[id]/history");
  }
}
