...
// names and raw error text. Only a failure's reason is read, to say what
// failed (computerHistoryLabel), and it stays here. Agent-run telemetry
// shares the table and is left out; it has its own Activity page.
...
import { COMPUTER_HISTORY_LABELS, COMPUTER_HISTORY_LIMIT, computerHistoryLabel } from "@/lib/hivra/computer-history";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      ...
      .select("event, created_at, reason:detail->>reason")
      .eq("agent_id", id)
      .eq("user_id", userId)
    ...
    const events = (Array.isArray(data) ? data : []).flatMap((row: { event?: unknown; created_at?: unknown; reason?: unknown }) => {
      const event = typeof row.event === "string" ? row.event : "";
      const label = computerHistoryLabel(event, row.reason);
