/**
 * One env-drifted PVE host must not abort the whole fleet vzdump batch.
 *
 ...
 * ops event and the dead-man heartbeat were skipped. Whole-VM backups for PAID
 * tenants then stopped fleet-wide with nothing naming the host at fault.
 */
...
describe("GET /api/cron/daily-vm-backups — one drifted host can't abort the fleet", () => {
  ...
  const failureEvent = (): any =>
    mockedOpsEvent.mock.calls
      .map((c) => c[0])
      .find((e) => e?.source === "cron.daily_vm_backups_failed");
  ...
  it("still backs up the healthy host, and reports the drifted one as a per-instance failure", async () => {
    ...
    // The healthy host's vzdump actually ran.
  ...
  it("still stamps the dead-man heartbeat", async () => {
    ...
    expect(mockedHeartbeat).toHaveBeenCalledWith("daily-vm-backups");
  });
