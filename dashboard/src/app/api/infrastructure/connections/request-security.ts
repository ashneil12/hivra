...
/** Like readBoundedJson, but returns the exact UTF-8 text, for a receiver that
 * must compare a repeat byte for byte before parsing it. */
export async function readBoundedText(
  request: NextRequest,
  maxBytes: number,
  timeoutMs: number,
): Promise<{ ok: true; text: string } | { ok: false; reason: "invalid" | "too_large" | "timeout" }> {
  const declaredLength = request.headers.get("content-length");
  if (declaredLength !== null) {
    const parsedLength = Number(declaredLength);
  ...
  try {
    ...
    return { ok: false, reason: timedOut ? "timeout" : "invalid" };
  } finally {
