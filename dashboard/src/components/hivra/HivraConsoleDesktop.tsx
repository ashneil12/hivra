      ...
      <div ref={viewportRef} aria-label={`${name} interactive desktop`} style={{ flex: 1, minHeight: 0, overflow: "hidden", background: "#000" }}>
        {profile === "windows" ? windowsLaunchUrl ? (
          <iframe key={windowsFrameRevision} ref={windowsFrameRef} title={`${name} Windows desktop`} src={windowsLaunchUrl}
            ...
            onLoad={() => { setWindowsFrameLoaded(true); setMessage("Windows desktop gateway loaded · Full screen is optional"); windowsFrameRef.current?.focus(); }}
            onErrorCapture={() => { setWindowsLaunchState("failed"); setMessage("Windows desktop could not load. Reconnect Windows to request a fresh session."); }}
