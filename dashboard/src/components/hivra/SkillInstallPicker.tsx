...
// covers only the visible viewport, so the close button and footer stay reachable.
...
@media (max-width: 767px) {
  .skill-picker-footer { padding-bottom: calc(14px + env(safe-area-inset-bottom, 0px)) !important; }
}
@media (max-width: 479px) {
  .skill-picker-actions { flex-wrap: wrap; width: 100%; }
  .skill-picker-actions > button { flex: 1 1 100%; justify-content: center; }
}
`;
...
export function SkillInstallPicker({
  ...
  return (
    <SafePortal>
      ...
      <PickerDialog onClose={onClose} closeOnEscape={!installing}>
        <div
        ...
        >
          ...
          <div className="skill-picker-footer" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "14px 22px", borderTop: "1px solid var(--etched-border)", flexWrap: "wrap" }}>
            ...
            <div className="skill-picker-actions" style={{ display: "flex", gap: 8 }}>
              <button type="button" onClick={onClose} disabled={installing} style={{ border: "1px solid var(--etched-border)", background: "transparent", color: "var(--text-secondary)", padding: "8px 16px", minHeight: 44, cursor: installing ? "default" : "pointer", fontSize: 13 }}>
                Cancel
              </button>
              <button
                type="button"
...
// Tab stays inside, Escape closes (not mid-install), and focus returns to the trigger.
