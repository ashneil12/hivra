...
export default function UseCasesSection() {
  ...
  return <section id="use-cases" className={`${styles.section} ${styles.useSection}`}>
    ...
    <div className={styles.sectionEnding}><p>{section.footer}</p><Link href={english ? "#computers" : "/get-started?plan=free"} className={styles.textLink}>{section.cta}<ArrowRight size={18} /></Link></div>
  </section>;
}
