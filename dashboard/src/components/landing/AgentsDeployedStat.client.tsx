...
export default function AgentsDeployedStatClient() {
  ...
  return <Link href="/stats" className="agents-deployed-stat"
    aria-label={`${RELAUNCH_AGENTS_TOTAL.toLocaleString(locale)} ${t.label}. ${t.fullStats.replace("→", "").trim()}`}
    ...
    <span style={{fontSize:".9rem",color:"var(--text-secondary)",fontWeight:500}}>{t.label}</span>
