...
export function EditorialCTA({ title, label = "Start Now" }: { title?: ReactNode; label?: string }) {
  return <section className={styles.cta}><div><h2>{title ?? <>Deploy in <strong>5 minutes.</strong></>}</h2><p>7-day money-back guarantee. BYO AI key. From $9.99/mo.</p></div><Link href={PUBLIC_START_HREF} className={styles.button}>{label}<ArrowUpRight size={20} aria-hidden="true" /></Link></section>;
}
