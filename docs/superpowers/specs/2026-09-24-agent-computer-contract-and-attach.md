...
| 6 | `20260925000300_hivra_agent_attach_interrupt.sql` | Additive | none (existing rows get `lease_released = false` and no interruption) |
...
**Existing computers and the terminal socket move.** Release 2026.09.24.2
...
| Id | Steps | Pass |
|---|---|---|
| AC-H1 | Launch a Hivra Cloud Codex agent; open its Terminal and its computer's Terminal | It launches; the launch writes through `insert_hivra_managed_agent` (no "Could not verify remaining compute"); Billing's agent count rises by one; both terminals open a shell through the page (the owner-only sockets of 2026.09.24.2) |
...
| AC-H7 | On an existing Canary computer made before 2026.09.24.2, Manage → Update & restart; then open its Terminal and computer Terminal | The update completes (or refuses with "terminal units are missing or unsafe" and changes nothing); both terminals open a shell afterwards |
