...
| 7 | `_pending_destructive_migrations/hivra_agent_slot_writer_guard.sql` | **Blocking** (queued) | only after the code serves there, with the launch, start and restart smoke tests (an agent in `error` included) before and after |

The provisioner release is 2026.09.24.3. 2026.09.24.2 and migration version
20260924220000 belong to the persistent-sessions release (PR #124), which is
not in `canary` yet; 2026.09.24.3 is built on 2026.09.24.1. Step 2 and #124's
admission each anchor on the 2026.09.24.1 entry, so either can apply first and
both releases stay admitted. When #124 lands first, this release has to be
re-sealed on top of its bundle before it merges.

Steps 1 to 6 go on before the merged code serves (Canary: before the merge
into `canary` builds; production: **before the owner's Promote**, not only on
Canary). Step 7 follows the served code on each environment. Each file is
...
| T3 | A stuck attach blocks the computer's stop or delete | Cancel before dispatch; observe-only reconciliation; complete and fail transitions. A step already sent lets the computer go when the host saw the VM not running under its lock, or when a delete is pending, without being marked done; it takes the computer back once it runs again, free and unchanged, and ends by what it shows; deleting the computer ends it. The queue is least recently tried first. Residual: a guest that runs but never answers keeps its step, and the computer's Start, Stop and Restart, held until the owner deletes the computer | New: `scripts/test-hivra-attachment-lifecycle.cjs` (complete, fail, detach, delete-while-claimed, VM-down-while-dispatched for an install and a Remove, delete-while-dispatched, the computer delete ending an interrupted step, the queue order); `attachment-worker.test.ts` (a stopped VM during activation, observation, staging, cleanup, Change access and Remove lets the computer go; a pending delete wins; an interrupted install is removed, never finished; a guest that does not answer stays held) |
