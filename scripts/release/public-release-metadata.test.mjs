...
test('every third-party GitHub Action is pinned to an immutable commit', () => {
  ...
  // 15: the attach fixtures job adds one pinned actions/checkout and one actions/setup-node.
  assert.equal(actionCount, 15, 'review the complete workflow action inventory when it changes');
});
